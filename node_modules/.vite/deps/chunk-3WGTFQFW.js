...
var ml = (c, b, l) => b in c ? Zl(c, b, { enumerable: true, configurable: true, writable: true, value: l }) : c[b] = l;
var S = (c, b, l) => (ml(c, typeof b != "symbol" ? b + "" : b, l), l);
var T = (c, b, l) => {
  if (!b.has(c))
    throw TypeError("Cannot " + l);
};
var W = (c, b, l) => (T(c, b, "read from private field"), l ? l.call(c) : b.get(c));
var J = (c, b, l) => {
  if (b.has(c))
    throw TypeError("Cannot add the same private member more than once");
  b instanceof WeakSet ? b.add(c) : b.set(c, l);
};
var i = (c, b, l, d) => (T(c, b, "write to private field"), d ? d.call(c, l) : b.set(c, l), l);
var H = (c, b, l, d) => ({
  set _(X) {
    i(c, b, X, l);
  },
...
var y = (c, b, l) => (T(c, b, "access private method"), l);
function cl(c) {
  const { selector: b, id: l } = c;
...
var Wl = (c) => {
  let b;
  return function() {
    let l = this, d = arguments;
    b && window.cancelAnimationFrame(b), b = window.requestAnimationFrame(function() {
      c.apply(l, d);
    });
  };
};
var Vl = (c = { dedupe: false }) => {
  const { dedupe: b } = c;
  let l = [], d;
  const X = (m) => (b && (l = []), l.push(m), d || (d = Z()), d.finally(() => {
    d = void 0;
  })), Z = async () => {
    const m = [];
    for (; l.length; ) {
...
var Jl = (c) => JSON.parse(JSON.stringify(c));
var q = class {
  /**
...
var Rl = (c) => {
  let b = false, l = c.slice(c.startsWith("#") ? 1 : 0);
  l.length === 3 ? l = [...l].map((X) => X + X).join("") : l.length === 8 && (b = true), l = parseInt(l, 16);
  let d = {
    r: l >>> 16,
    g: (l & 65280) >>> 8,
    b: l & 255
...
var il = class {
  constructor(b, l = {}) {
    ...
    J(this, w);
    J(this, C);
    S(this, "rollCollectionData", {});
    S(this, "rollGroupData", {});
    S(this, "rollDiceData", {});
    S(this, "themeData", []);
    S(this, "themesLoadedData", {});
    J(this, F, 0);
    J(this, f, 0);
    J(this, x, 0);
    J(this, e, 0);
    ...
    const { onDieComplete: d, onRollComplete: X, onRemoveComplete: Z, onThemeConfigLoaded: m, onThemeLoaded: V, ...G } = l;
    this.config = { ...ul, ...G }, this.onDieComplete = l.onDieComplete || this.noop, this.onRollComplete = l.onRollComplete || this.noop, this.onRemoveComplete = l.onRemoveComplete || this.noop, this.onThemeLoaded = l.onThemeLoaded || this.noop, this.onThemeConfigLoaded = l.onThemeConfigLoaded || this.noop, this.canvas = cl({
      selector: b,
      id: this.config.id
    }), this.isVisible = true, this.loadThemeQueue = Vl({ dedupe: true });
  }
  resizeWorld() {
    const l = Wl(() => {
      W(this, u).resize({ width: this.canvas.clientWidth, height: this.canvas.clientHeight }), W(this, a).postMessage({ action: "resize", width: this.canvas.clientWidth, height: this.canvas.clientHeight });
    });
    window.addEventListener("resize", l);
  }
  async init() {
    return y(this, P, ll).call(this), await y(this, Q, _).call(this), this.resizeWorld(), W(this, u).onRollResult = (b) => {
      const l = this.rollDiceData[b.rollId], d = this.rollGroupData[l.groupId], X = this.rollCollectionData[l.collectionId];
      d.rolls[l.rollId].value = b.value, X.completedRolls++, X.completedRolls == X.rolls.length && X.resolve(Object.values(X.rolls).map(({ collectionId: G, id: Y, meshName: R, ...v }) => v));
      const { collectionId: Z, id: m, ...V } = l;
      this.onDieComplete(V);
    }, W(this, u).onRollComplete = () => {
      this.onRollComplete(this.getRollResults());
    }, W(this, u).onDieRemoved = (b) => {
      let l = this.rollDiceData[b];
      const d = this.rollCollectionData[l.removeCollectionId];
      d.completedRolls++, delete this.rollDiceData[l.rollId];
      const X = this.rollGroupData[l.groupId];
      delete X.rolls[l.rollId];
      const Z = y(this, C, D).call(this, l.groupId);
      X.value = Z.value, X.qty = Z.rollsArray.length, d.completedRolls == d.rolls.length && d.resolve(Object.values(d.rolls).map(({ id: v, ...L }) => L));
      const { collectionId: m, id: V, removeCollectionId: G, meshName: Y, ...R } = l;
      this.onRemoveComplete(R);
    }, await Promise.all([W(this, K), W(this, t)]), y(this, j, bl).call(this), await this.loadThemeQueue.push(() => this.loadTheme(this.config.theme)), this;
  }
  ...
  async getThemeConfig(b) {
    ...
    } : d = await fetch(`${l}/theme.config.json`).then((m) => {
      if (m.ok) {
        ...
        throw new Error(`Unable to fetch config file for theme: '${b}'. Request rejected with status ${m.status}: ${m.statusText}`);
    }).catch((m) => console.error(m));
    let X = this.config.origin + this.config.assetPath + this.config.meshFile, Z = "default";
    if (!d)
      throw new Error("No theme config data to work with.");
    if (d.hasOwnProperty("meshFile") && (X = `${l}/${d.meshFile}`, d.hasOwnProperty("meshName") ? Z = d.meshName : (console.warn("You should provide a meshName in your theme.config.json file"), Z = d.meshFile.replace(/(.*)\..{2,4}$/, "$1"))), d.hasOwnProperty("diceAvailable") || (d.diceAvailable = ["d4", "d6", "d8", "d10", "d12", "d20", "d100"]), d.hasOwnProperty("extends")) {
      let m = this.themesLoadedData[d.extends];
      m || (m = await this.loadTheme(d.extends).catch((V) => console.error(V))), m && (d.diceInherited = [...d.diceInherited || []], m.diceAvailable.map((V) => {
        d.diceInherited[V] = m.systemName;
      }));
    }
  ...
  async loadTheme(b) {
    if (this.themesLoadedData[b])
      return this.themesLoadedData[b];
    const l = this.themesLoadedData[b] = await this.getThemeConfig(b).catch((d) => console.error(d));
    if (this.onThemeConfigLoaded(l), !!l)
      return await W(this, u).loadTheme(l).catch((d) => console.error(d)), this.onThemeLoaded(l), l;
  }
  // TODO: use getter and setter
  // change config options
  async updateConfig(b) {
    const l = { ...this.config, ...b };
    return await this.loadThemeQueue.push(() => this.loadTheme(l.theme)), this.config = l, W(this, u).updateConfig(l), W(this, a).postMessage({
      action: "updateConfig",
      options: l
    }), this;
  }
  clear() {
    return i(this, F, 0), i(this, f, 0), i(this, x, 0), i(this, e, 0), this.rollCollectionData = {}, this.rollGroupData = {}, this.rollDiceData = {}, W(this, u).clear(), W(this, a).postMessage({ action: "clearDice" }), this;
  }
  hide() {
    return this.canvas.style.display = "none", this.isVisible = false, this;
  }
  show() {
    return this.canvas.style.display = "block", this.isVisible = true, this.resizeWorld(), this;
  }
  // TODO: pass data with roll - such as roll name. Passed back at the end in the results
  roll(b, { theme: l = this.config.theme, themeColor: d = this.config.themeColor, newStartPoint: X = true } = {}) {
    this.clear();
    const Z = H(this, F)._++;
    this.rollCollectionData[Z] = new g({
      id: Z,
      notation: b,
      theme: l,
      themeColor: d,
    ...
    const m = this.createNotationArray(b, this.themesLoadedData[l].diceAvailable);
    return y(this, w, M).call(this, m, Z), this.rollCollectionData[Z].promise;
  }
  add(b, { theme: l = this.config.theme, themeColor: d = this.config.themeColor, newStartPoint: X = true } = {}) {
    const Z = H(this, F)._++;
    this.rollCollectionData[Z] = new g({
      id: Z,
      notation: b,
  ...
  reroll(b, { remove: l = false, hide: d = false, newStartPoint: X = true } = {}) {
    const m = (Array.isArray(b) ? b : [b]).map(({ value: V, ...G }) => G);
    return l === true && this.remove(m, { hide: d }), this.add(m, { newStartPoint: X });
  }
  remove(b, { hide: l = false } = {}) {
    const d = Array.isArray(b) ? b : [b], X = H(this, F)._++;
    return this.rollCollectionData[X] = new g({
      id: X,
      notation: b,
      rolls: d
    }), d.map((Z) => {
      ...
      W(this, u).remove({ id: m, rollId: Z.rollId }), W(this, a).postMessage({ action: "removeDie", id: m });
  ...
  createNotationArray(b, l) {
    ...
    const Z = (G) => {
      ...
      G.hasOwnProperty("rollId") && this.rollDiceData.hasOwnProperty(G.rollId) && (G.rollId = m(G.rollId));
    };
    return d.forEach((G) => {
      typeof G == "string" ? X.push(this.parse(G, l)) : typeof d == "object" && (V(G), Z(G) && X.push(G));
    }), X;
  }
  // parse text die notation such as 2d10+3 => {number:2, type:6, modifier:3}
  // taken from https://github.com/ChapelR/dice-notation
  parse(b, l) {
    const d = /(\d+)([dD]{1}\d+)(.*)$/i, X = /(\d+)[dD]([0%]+)(.*)$/i, Z = /(\d+)[dD](f+[ate]*)(.*)$/i, m = /(\d+)[dD]([\d\w]+)([+-]{0,1}\d+)?/i, V = /([+-])(\d+)/, G = b.trim().replace(/\s+/g, ""), Y = (p, o) => {
      if (p = Number(p), Number.isNaN(p) || !Number.isInteger(p) || p < 1)
        throw new Error(o);
      return p;
    }, R = G.match(X) || G.match(d) || G.match(Z) || G.match(m);
    let v = 0;
    const L = "Invalid notation: " + b;
    ...
    return G.match(X) ? (s.sides = R[2], s.data = "single") : G.match(Z) ? s.sides = "fate" : (l.includes(G.match(m)[2]), s.sides = R[2]), s;
  }
  getRollResults() {
    return Object.entries(this.rollGroupData).map(([b, l]) => {
      const d = y(this, C, D).call(this, b);
      l.value = d.value, l.qty = d.rollsArray.length;
      const X = { ...l };
      return X.rolls = d.rollsArray, X;
    });
  }
};
F = /* @__PURE__ */ new WeakMap(), f = /* @__PURE__ */ new WeakMap(), x = /* @__PURE__ */ new WeakMap(), e = /* @__PURE__ */ new WeakMap(), u = /* @__PURE__ */ new WeakMap(), K = /* @__PURE__ */ new WeakMap(), z = /* @__PURE__ */ new WeakMap(), a = /* @__PURE__ */ new WeakMap(), t = /* @__PURE__ */ new WeakMap(), B = /* @__PURE__ */ new WeakMap(), Q = /* @__PURE__ */ new WeakSet(), _ = async function() {
  i(this, K, new Promise((l, d) => {
    i(this, z, l);
  }));
  const b = () => {
    W(this, z).call(this);
  ...
  if ("OffscreenCanvas" in window && "transferControlToOffscreen" in this.canvas && this.config.offscreen) {
    const l = await import("./world.offscreen-D3BE7TOE.js").then((d) => d.default);
    i(this, u, new l({
      canvas: this.canvas,
      options: this.config,
      onInitComplete: b
    ...
    const l = await import("./world.onscreen-UXJ6G4MJ.js").then((d) => d.default);
    i(this, u, new l({
      canvas: this.canvas,
      options: this.config,
      onInitComplete: b
...
}, P = /* @__PURE__ */ new WeakSet(), ll = function() {
  i(this, a, new Gl()), i(this, t, new Promise((b, l) => {
    i(this, B, b);
  })), W(this, a).onmessage = (b) => {
...
}, j = /* @__PURE__ */ new WeakSet(), bl = function() {
  const b = new MessageChannel();
  W(this, u).connect(b.port1), W(this, a).postMessage({
    action: "connect"
  }, [b.port2]);
}, w = /* @__PURE__ */ new WeakSet(), M = async function(b, l) {
  const d = this.rollCollectionData[l];
  let X = d.newStartPoint;
  b.forEach(async (Z) => {
    if (!Z.sides)
      throw new Error("Improper dice notation or unable to parse notation");
    const m = Z.theme || d.theme || this.config.theme, V = Z.themeColor || d.themeColor || this.config.themeColor, G = {}, Y = Z.groupId !== void 0;
    let R;
    const v = () => this.loadTheme(m);
    await this.loadThemeQueue.push(v);
    const { meshName: L, diceAvailable: s, diceInherited: p = {}, material: { type: o } } = this.themesLoadedData[m], I = Object.keys(p);
    let r = "", U;
    o === "color" && (U = Rl(V), r = U.r * 0.299 + U.g * 0.587 + U.b * 0.114 > 175 ? "_dark" : "_light");
    for (var A = 0, dl = Z.qty; A < dl; A++) {
      let k = Z.rollId !== void 0 ? Z.rollId : H(this, x)._++, Xl = Z.id !== void 0 ? Z.id : H(this, e)._++;
      R = Y ? Z.groupId : W(this, f);
      const N = Number.isInteger(Z.sides) ? `d${Z.sides}` : Z.sides, h = {
        sides: Z.sides,
        data: Z.data,
        dieType: N,
        groupId: R,
      ...
      if (G[k] = h, this.rollDiceData[k] = h, d.rolls.push(this.rollDiceData[k]), h.sides === "fate" && !s.includes(N) && !I.includes(N)) {
        console.warn(`fate die unavailable in '${m}' theme. Using fallback.`);
        const n = -1, E = 1;
        h.value = q.range(n, E), W(this, u).addNonDie(h);
      } else if (this.config.suspendSimulation || !s.includes(N) && !I.includes(N)) {
        console.warn(this.config.suspendSimulation ? "3D simulation suspended. Using fallback." : `${h.sides} die unavailable in '${m}' theme. Using fallback.`);
        const n = Number.isInteger(h.sides) ? h.sides : parseInt(h.sides.replace(/\D/g, ""));
        h.value = q.range(1, n), W(this, u).addNonDie(h);
      } else {
        let n;
        if (I.includes(N)) {
          const E = p[N];
          n = this.themesLoadedData[E];
        }
        W(this, u).add({
          ...h,
          newStartPoint: X,
    ...
    Y ? Object.assign(this.rollGroupData[R].rolls, G) : (Z.rolls = G, Z.id = R, this.rollGroupData[R] = Z, ++H(this, f)._);
  });
}, C = /* @__PURE__ */ new WeakSet(), D = function(b) {
  const l = this.rollGroupData[b], d = Object.values(l.rolls).map(({ collectionId: Z, id: m, meshName: V, ...G }) => G);
  let X = d.reduce((Z, m) => {
    const V = isNaN(m.value) ? 0 : m.value;
    return Z + V;
  }, 0);
  return X += l.modifier ? l.modifier : 0, { value: X, rollsArray: d };
};
var g = class {
  constructor(b) {
    Object.assign(this, b), this.rolls = b.rolls || [], this.completedRolls = 0;
    const l = this;
    this.promise = new Promise((d, X) => {
      l.resolve = d, l.reject = X;
    });
  }
};
...
//# sourceMappingURL=chunk-3WGTFQFW.js.map
